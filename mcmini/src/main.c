...
int main(void) {
    ...
    // Test with a 4pi source spreading equally in all directions.
    Source4Pi src = {
        .center = vec3(0.0, 0.0, 0.0), // m, place source slightly before origin to avoid immediate self-intersection with sample at origin

        .lambda0 = 4.04,    // Angstrom
        .dlambda = 0.25,    // Angstrom half-width (=> [3.9, 4.1])
        .flux = 1e14,       // arbitrary units (since we will normalize monitor by n_history)
        .gauss = 0         // 0=flat, 1=gaussian distribution for energy and wavelength
    };
...
// // Match McStas Source_div settings exactly
    ...
    // the monitorsphere used in the 4pi source test
    MonitorSphere mon;
    if (!monitor_sphere_open_binned(&mon, "4pisrc_to_4pimon.csv",
    ...
    for (long long i = 0; i < Nsim; i++) {
        Particle p = source_4pi_emit(&src, &rng);
        monitor_sphere_record(&mon, &p);
    }

    monitor_sphere_normalize_per_history(&mon); // optional, if wanted
    monitor_sphere_close(&mon); // just test open and close for now. Will add recording and normalization later.
