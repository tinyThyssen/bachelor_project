...
int main(void) {
    ...
    SourceSimple src = {
        .center = vec3(0.0, 0.0, -0.2), // m, place source slightly before origin to avoid immediate self-intersection with sample at origin
        // .radius = 0.04,      // m, cylinder_height/2 to match solid_cylinder.instr
        .xwidth = 0.03,      // m, horizontal width of rectangular source area
        .yheight = 0.03,    // m, vertical height of rectangular source area
        .dist = 1.0,       // m
...
// // Match McStas Source_div settings exactly
    ...
    long long Nsim = 1e10; // number of simulated neutrons
    ...
    //     "solid_cyl_r0p5h2_scatter_6.csv",
    //     "solid_cyl_r0p5h2_scatter_7.csv",
    //     "solid_cyl_r0p5h2_scatter_8.csv"
    // };
    // for (int s = 0; s < 8; s++) {
